...
# constants
dist_threshold = 3.0  # distance threshold for pipe proximity
centerline_angle_threshold = math.radians(
    10
)  # angle threshold for two pipes to be considered parallel
centerline_dist_threshold = (
    0.001  # centerline distance threshold pipes to be to considered connected
)
...
def get_centerline_distance(a, b):
    ...
    centerline_distance = abs(
        np.dot(centerline_connecting_line, center_connecting_line)
...
def get_distance_to_intersection(a, b):
    ...
    sq_mag_ccl = np.dot(centerline_connecting_line, centerline_connecting_line)
    t1 = (
        np.dot(np.cross(b[3], centerline_connecting_line), center_connecting_line)
        / sq_mag_ccl
    )
    t2 = (
        np.dot(np.cross(a[3], centerline_connecting_line), center_connecting_line)
        / sq_mag_ccl
    )
...
def visualise_tees(tee_connections, blueprint, pipes, pipe_edges, return_type="cloud"):
    ...
    for tee in tqdm(tee_connections):
        ...
        r1 = (
            get_radius_from_bbox(pipe_pair[0][2])
            + get_radius_from_bbox(pipe_pair[1][2])
        ) / 2
        r2 = get_radius_from_bbox(other[2])
        # print('r', r1, r2)

        l1_edges = [
            pipe_edges[pipe_pair_ids[0]][pipe_edge_ids[0]],
            pipe_edges[pipe_pair_ids[1]][pipe_edge_ids[1]],
        ]
        l1 = np.sqrt(sq_dist_vect(l1_edges[0], l1_edges[1]))
        l2_edges = [
            (l1_edges[0] + l1_edges[1]) / 2,
            pipe_edges[other_id][other_edge_id],
        ]
...
def get_elbow_params_from_pipes(el, pipes, pipe_edges):
    ...
    r = (get_radius_from_bbox(a[2]) + get_radius_from_bbox(b[2])) / 2
    p = a_edge.tolist()
    ...
    d3 = -1.0 * norm_array((norm_array(d2) - norm_array(d1) * ang_cos) / (1 - ang_cos))
