...
from src.graph import (
    process_nodes,
    process_edges,
    IndustrialFacilityDataset,
    get_node_features,
    get_edges_from_node_info,
)
from src.geometry import sq_dist_vect, vector_mag
from src.centerline import get_centerline_deviation


def check_predictions_fast(
    preds, point_info, node_info, dist_thresh=0.0002, rough_dist_thresh=0.2
):
    refined_preds = []
    discarded_preds = []
    rough_count = 0
    ...
    for pair in tqdm(preds):
        ...
        if (
            (sq_dist_vect(bb0[0], bb1[0]) < rough_dist_thresh)
            or (sq_dist_vect(bb0[0], bb1[1]) < rough_dist_thresh)
            or (sq_dist_vect(bb0[1], bb1[0]) < rough_dist_thresh)
            or (sq_dist_vect(bb0[1], bb1[1]) < rough_dist_thresh)
        ):
            # slower, precise check using points
            rough_count += 1
            dist = np.min(
                distance.cdist(point_info[pair[0]], point_info[pair[1]], "sqeuclidean")
            )
            if dist < dist_thresh:
...
def connectivity_refinements(
    preds,
    node_info,
    params_path,
    dataset,
    radius_threshold,
    centerline_angle_threshold,
