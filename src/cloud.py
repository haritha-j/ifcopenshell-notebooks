...
def add_noise(cl, noise_size, rng):
    ...
    noise = np.column_stack(
        [rng.uniform(min_point[i], max_point[i], noise_size) for i in range(3)]
    )
...
def element_to_cloud(element, save_path=None, density=0):
    ...
    for j in range(0, point_count, 3):
        if j < limit:
            ...
            centroids.extend(
                sample_points(
                    (boundaries[j]), (boundaries[j + 1]), (boundaries[j + 2]), samples
                )
            )
    ...
    if density > 0:
        boundaries = boundaries[
            np.random.choice(boundaries.shape[0], density, replace=False), :
        ]
