...
def get_corner(obb, center_other, edge_distance):
    ...
    candidate1 = [
        get_point_along_axis(center[i], direction[i], obb[1], edge_distance)
        for i in range(3)
    ]
    candidate2 = [
        get_point_along_axis(center[i], direction[i], -1 * obb[1], -1 * edge_distance)
        for i in range(3)
    ]
...
def get_oriented_bbox_from_points(points):
    ...
    l2 = math.sqrt(
        sq_distance(
            bbox[0][0], bbox[0][1], bbox[0][2], bbox[3][0], bbox[3][1], bbox[3][2]
        )
    )
    l3 = math.sqrt(
        sq_distance(
            bbox[0][0], bbox[0][1], bbox[0][2], bbox[4][0], bbox[4][1], bbox[4][2]
        )
    )
    ...
    if dominant_axis == 0:
        dominant_direction = vector_normalise(
            [bbox[0][i] - bbox[1][i] for i in range(3)]
        )
    elif dominant_axis == 1:
        dominant_direction = vector_normalise(
            [bbox[0][i] - bbox[3][i] for i in range(3)]
        )
    else:
        dominant_direction = vector_normalise(
            [bbox[0][i] - bbox[4][i] for i in range(3)]
        )
    ...
    # print(center)
    return [dominant_direction, max(half_lengths), half_lengths, center]
...
def get_labelcloud_bbox(points, label):
    ...
    bbox = {
        "name": label,
        "centroid": {"x": c[0], "y": c[1], "z": c[2]},
        "dimensions": {"length": e[0], "width": e[1], "height": e[2]},
